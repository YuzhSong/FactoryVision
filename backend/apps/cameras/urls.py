from django.urls import path

from .views import (
    camera_create_view,
    camera_delete_view,
    camera_list_view,
    camera_stream_start_view,
    camera_stream_status_view,
    camera_stream_stop_view,
    camera_toggle_view,
    camera_update_view,
    placeholder_view,
)

urlpatterns = [
    path("placeholder/", placeholder_view, name="cameras-placeholder"),
    path("", camera_create_view, name="camera-create"),
    path("list/", camera_list_view, name="camera-list"),
    path("<int:camera_id>/stream/status/", camera_stream_status_view, name="camera-stream-status"),
    path("<int:camera_id>/stream/start/", camera_stream_start_view, name="camera-stream-start"),
    path("<int:camera_id>/stream/stop/", camera_stream_stop_view, name="camera-stream-stop"),
    path("<int:camera_id>/", camera_update_view, name="camera-update"),
    path("<int:camera_id>/toggle/", camera_toggle_view, name="camera-toggle"),
    path("<int:camera_id>/delete/", camera_delete_view, name="camera-delete"),
]
