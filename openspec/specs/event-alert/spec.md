...
The alert list endpoint SHALL support keyword search on title, severity/status/cameraId filtering, time-range filtering on occurred_at, and pagination.

#### Scenario: Search alerts by keyword

- **GIVEN** the user is authenticated and alerts exist
- **WHEN** `GET /api/alerts/list/?keyword=区域&page=1&pageSize=20` is called
- **THEN** the backend SHALL return alerts whose `title` contains the keyword (case-insensitive, Chinese compatible)
...
- **THEN** the backend SHALL return only alerts within the specified time range

#### Scenario: Combined filter

- **GIVEN** the user applies multiple filters simultaneously
- **WHEN** keyword, severity, status, and time range are all specified
