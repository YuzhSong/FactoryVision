...
class FallDetector:
    ...
    def detect(self, track_history):
        ...
        elif not transition["hasDrop"] and not (sustained_low and sustained_low["hasSustainedLow"]):
    ...
    def _sustained_low_posture_evidence(self, baseline, recent, frame_scores):
        ...
        very_low = max_height_drop >= self.sustained_low_drop_ratio and any(
            (self._bbox(entry.get("bbox"))[3] - self._bbox(entry.get("bbox"))[1]) <= base_height * self.very_low_height_ratio
            for entry in recent
            if self._bbox(entry.get("bbox")) is not None
        )
