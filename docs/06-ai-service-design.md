...
`ai-service` 当前提供 Flask 健康检查接口和 AI 模块骨架。异常行为识别相关模块已提供规则型本地实现，包括人员检测结果格式化、头盔异常结果格式化、危险区域 footPoint 判断、摔倒宽高比判断、异常跑动连续帧速度判断，以及 AI 结果上报客户端。真实 YOLO 模型加载、视频帧读取、人脸识别推理仍为 `planned`。
